...
namespace PowerPoint02AddinCS4
{
    public partial class SamplePane : UserControl, NetOffice.PowerPointApi.Tools.ITaskPane // Not necessary to implement ITaskPane but its helpful
    {
        ...
        public void OnVisibleStateChanged(bool visible)
        {
            ...
            // To avoid slow down the PowerPoint startup sequence - we create them on demand when user want show the pane
            // (Real world code want doing that async)
            if (visible && null == Counter)
            {
                Counter = new PerformanceCounter("Process", "% Processor Time", "POWERPNT");
