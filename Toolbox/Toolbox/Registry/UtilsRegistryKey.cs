    ...
    public class UtilsRegistryKey
    {
        ...
        public void CreateNewSubKey()
        {
            ...
            if (path.StartsWith("HKEY_CURRENT_USER"))
            {
                ...
                RegistryKey key = Registry.CurrentUser.CreateSubKey(path + "\\" + name);
                key.Close();
            }
            else
            {
                ...
                //path = path.Substring("HKEY_LOCAL_MACHINE\\".Length); + path + "\\"
        ...
        public RegistryKey Open(bool writable)
        {
            ...
            if (null != Parent)
            {
                RegistryKey parentKey = Parent.Open(writable);
                RegistryKey key = parentKey.OpenSubKey(Name, writable);
                parentKey.Close();
                return key;
            }
            else
            {
                RegistryKey key = _root.Key.InnerKey.OpenSubKey(Path, writable);
                return key;
            }
             */
        }
        ...
        public RegistryKey Open()
        {
            ...
            if (null != Parent)
            {
                RegistryKey parentKey = Parent.Open();
