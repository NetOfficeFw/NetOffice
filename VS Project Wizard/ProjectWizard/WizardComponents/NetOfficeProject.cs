...
namespace NetOffice.ProjectWizard
{
    ...
    {   
        ...
        internal TargetLanguage TargetLanguage
        {
            get
            {
        ...
        protected internal void CopyAssemblies()
        {
            ...
            string rootPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "NetOffice Assemblies");
            if (!Directory.Exists(rootPath))
                throw new DirectoryNotFoundException("NetOfficeAssemblies folder not exists or deleted.")
                    ;
            foreach (string item in _neededNetOfficeAssemblies)
            {
                ...
                string sourceDocuFile = string.Format("{2}\\Documentation Files\\{1}", _targetRuntime, item + ".xml", rootPath);
