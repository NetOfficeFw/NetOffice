...
namespace NetOffice.MSHTMLApi
{
	...
	public class HTMLDocument : DispHTMLDocument,IEventBinding
	{
		...
		/// <summary>
        /// returns all running MSHTML.HTMLDocument objects from the running object table(ROT)
        /// </summary>
        /// <returns>an MSHTML.HTMLDocument array</returns>
		public static NetOffice.MSHTMLApi.HTMLDocument[] GetActiveInstances()
		{		
			NetRuntimeSystem.Collections.Generic.List<object> proxyList = NetOffice.RunningObjectTable.GetActiveProxiesFromROT("MSHTML","HTMLDocument");
		...
		/// <summary>
        /// returns a running MSHTML.HTMLDocument object from the running object table(ROT). the method takes the first element from the table
        /// </summary>
        /// <returns>an MSHTML.HTMLDocument object or null</returns>
		public static NetOffice.MSHTMLApi.HTMLDocument GetActiveInstance()
		{
			object proxy = NetOffice.RunningObjectTable.GetActiveProxyFromROT("MSHTML","HTMLDocument", false);
			if(null != proxy)
		...
		{
			object proxy = NetOffice.RunningObjectTable.GetActiveProxyFromROT("MSHTML","HTMLDocument", throwOnError);
			if(null != proxy)
				return new NetOffice.MSHTMLApi.HTMLDocument(null, proxy);
			else
				return null;
