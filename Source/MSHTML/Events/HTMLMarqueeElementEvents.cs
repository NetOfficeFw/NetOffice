...
{	
    ...
    [ComImport, Guid("3050F2B8-98B5-11CF-BB82-00AA00BDCE0B"), InterfaceType(ComInterfaceType.InterfaceIsIDispatch), TypeLibType((short)0x1010)]
	...
	public class HTMLMarqueeElementEvents_SinkHelper : SinkHelper, HTMLMarqueeElementEvents
	{
		#region Static
		
		public static readonly string Id = "3050F2B8-98B5-11CF-BB82-00AA00BDCE0B";
		...
		public HTMLMarqueeElementEvents_SinkHelper(ICOMObject eventClass, IConnectionPoint connectPoint): base(eventClass)
		{
		...
		#region HTMLMarqueeElementEvents Members
		...
		{
			Delegate[] recipients = _eventBinding.GetEventRecipients("onbounce");
			if( (true == _eventClass.IsCurrentlyDisposing) || (recipients.Length == 0) )
			{
				Invoker.ReleaseParamsArray();
				return;
			...
			object[] paramsArray = new object[0];
			_eventBinding.RaiseCustomEvent("onbounce", ref paramsArray);
		}

		public void onfinish()
		{
			Delegate[] recipients = _eventBinding.GetEventRecipients("onfinish");
			if( (true == _eventClass.IsCurrentlyDisposing) || (recipients.Length == 0) )
			{
				Invoker.ReleaseParamsArray();
				return;
			...
			object[] paramsArray = new object[0];
			_eventBinding.RaiseCustomEvent("onfinish", ref paramsArray);
		}

		public void onstart()
		{
			Delegate[] recipients = _eventBinding.GetEventRecipients("onstart");
			if( (true == _eventClass.IsCurrentlyDisposing) || (recipients.Length == 0) )
			{
				Invoker.ReleaseParamsArray();
				return;
			...
			object[] paramsArray = new object[0];
			_eventBinding.RaiseCustomEvent("onstart", ref paramsArray);
		}
