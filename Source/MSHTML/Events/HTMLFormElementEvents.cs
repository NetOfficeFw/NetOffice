...
{	
    ...
    [ComImport, Guid("3050F364-98B5-11CF-BB82-00AA00BDCE0B"), InterfaceType(ComInterfaceType.InterfaceIsIDispatch), TypeLibType((short)0x1010)]
	public interface HTMLFormElementEvents
	{
		...
		[SupportByVersion("MSHTML", 4)]
		[PreserveSig, MethodImpl(MethodImplOptions.InternalCall, MethodCodeType = MethodCodeType.Runtime), DispId(1015)]
		void onreset();
	}
	...
	public class HTMLFormElementEvents_SinkHelper : SinkHelper, HTMLFormElementEvents
	{
		#region Static
		
		public static readonly string Id = "3050F364-98B5-11CF-BB82-00AA00BDCE0B";
		...
		public HTMLFormElementEvents_SinkHelper(ICOMObject eventClass, IConnectionPoint connectPoint): base(eventClass)
		{
		...
		#region HTMLFormElementEvents Members
		...
		{
			Delegate[] recipients = _eventBinding.GetEventRecipients("onsubmit");
			if( (true == _eventClass.IsCurrentlyDisposing) || (recipients.Length == 0) )
			{
				Invoker.ReleaseParamsArray();
				return;
			...
			object[] paramsArray = new object[0];
			_eventBinding.RaiseCustomEvent("onsubmit", ref paramsArray);
		}

		public void onreset()
		{
			Delegate[] recipients = _eventBinding.GetEventRecipients("onreset");
			if( (true == _eventClass.IsCurrentlyDisposing) || (recipients.Length == 0) )
			{
				Invoker.ReleaseParamsArray();
				return;
			...
			object[] paramsArray = new object[0];
			_eventBinding.RaiseCustomEvent("onreset", ref paramsArray);
		}
