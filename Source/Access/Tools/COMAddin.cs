    ...
    public abstract class COMAddin : COMAddinBase, IOfficeCOMAddin
    {
        ...
        void NetOffice.Tools.Native.IDTExtensibility2.OnConnection(object application, ext_ConnectMode ConnectMode, object AddInInst, ref Array custom)
        {
            try
            {
                ...
                this.Application = Factory.CreateKnownObjectFromComProxy<Access.Application>(null, application, typeof(Access.Application));
                Utils = OnCreateUtils();
        ...
        public virtual void CTPFactoryAvailable(object CTPFactoryInst)
        {
            try
            {
                ...
                TaskPaneFactory = paneHandler.CreateCustomPanes<ITaskPane, Access.Application>(Factory, CTPFactoryInst, TaskPanes, OnError, Application);
            }
            catch (NetRuntimeSystem.Exception exception)
            {
                Factory.Console.WriteException(exception);
