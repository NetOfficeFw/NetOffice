...
{
    /// <summary>
    /// The class provides a lot of essential functionality for an MS-Access COMAddin
    /// </summary>
	[ComVisible(true)]
    public abstract class COMAddin : IDTExtensibility2, Office.IRibbonExtensibility, Office.ICustomTaskPaneConsumer
    {
        ...
        protected static bool SetTweakPersistenceEntry(Type addinType, string name, string value, bool throwException)
        {
            try
            {
                ...
                RegistryKey regKeyAccess = null;
                if (registry.Value == RegistrySaveLocation.LocalMachine)
                    regKeyAccess = Registry.LocalMachine.OpenSubKey(_addinOfficeRegistryKey + progID.Value, true);
                else
                    regKeyAccess = Registry.CurrentUser.OpenSubKey(_addinOfficeRegistryKey + progID.Value, true);

                if (null == regKeyAccess)
                    regKeyAccess = Registry.CurrentUser.CreateSubKey(_addinOfficeRegistryKey + progID.Value);
                if (null == regKeyAccess)
                    return false;
       ...
       /// <param name="methodKind">origin method where the error comes from</param>
        ...
        public static void RegisterFunction(Type type)
        {
            try                
            {
                ...
                regKeyAccess.SetValue("LoadBehavior", addin.LoadBehavior);
                regKeyAccess.SetValue("FriendlyName", addin.Name);
                regKeyAccess.SetValue("Description", addin.Description);
                if(-1 != addin.CommandLineSafe)
                    regKeyAccess.SetValue("CommandLineSafe", addin.CommandLineSafe);
