    ...
    public abstract class COMAddin : COMAddinBase, IDTExtensibility2, Office.IRibbonExtensibility, Office.ICustomTaskPaneConsumer
    {
        ...
        protected static bool SetTweakPersistenceEntry(Type addinType, string name, string value, bool throwException)
        {
            try
            {
                ...
                RegistryKey regKeyExcel = null;
                if (registry.Value == RegistrySaveLocation.LocalMachine)
                    regKeyExcel = Registry.LocalMachine.OpenSubKey(_addinOfficeRegistryKey + progID.Value, true);
                else
                    regKeyExcel = Registry.CurrentUser.OpenSubKey(_addinOfficeRegistryKey + progID.Value, true);

                if (null == regKeyExcel)
                    regKeyExcel = Registry.CurrentUser.CreateSubKey(_addinOfficeRegistryKey + progID.Value);
                if (null == regKeyExcel)
                    return false;
       ...
       /// <param name="methodKind">origin method where the error comes from</param>
        ...
        public static void RegisterFunction(Type type)
        {
            try                
            {
                ...
                regKeyExcel.SetValue("LoadBehavior", addin.LoadBehavior);
                regKeyExcel.SetValue("FriendlyName", addin.Name);
                regKeyExcel.SetValue("Description", addin.Description);
                if(-1 != addin.CommandLineSafe)
                    regKeyExcel.SetValue("CommandLineSafe", addin.CommandLineSafe);
