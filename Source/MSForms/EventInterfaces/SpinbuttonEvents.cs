...
{	
	...
	[ComImport, Guid("79176FB2-B7F2-11CE-97EF-00AA006D2776"), InterfaceType(ComInterfaceType.InterfaceIsIDispatch), TypeLibType((short)0x1010)]
	public interface SpinbuttonEvents
	{
		[SupportByVersionAttribute("MSForms", 2)]
	...
	public class SpinbuttonEvents_SinkHelper : SinkHelper, SpinbuttonEvents
	{
		#region Static
		...
		public SpinbuttonEvents_SinkHelper(COMObject eventClass, IConnectionPoint connectPoint): base(eventClass)
		{
		...
		#region SpinbuttonEvents Members
		...
		public void SpinUp()
		{
			Delegate[] recipients = _eventBinding.GetEventRecipients("SpinUp");
			if( (true == _eventClass.IsCurrentlyDisposing) || (recipients.Length == 0) )
			{
				Invoker.ReleaseParamsArray();
				return;
			...
			object[] paramsArray = new object[0];
			_eventBinding.RaiseCustomEvent("SpinUp", ref paramsArray);
		}

		public void SpinDown()
		{
			Delegate[] recipients = _eventBinding.GetEventRecipients("SpinDown");
			if( (true == _eventClass.IsCurrentlyDisposing) || (recipients.Length == 0) )
			{
				Invoker.ReleaseParamsArray();
				return;
			...
			object[] paramsArray = new object[0];
			_eventBinding.RaiseCustomEvent("SpinDown", ref paramsArray);
		}
