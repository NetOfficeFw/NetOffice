...
{
    /// <summary>
    /// The class provides a lot of essential functionality for an MS-PowerPoint COMAddin
    /// </summary>
	[ComVisible(true)]
    public abstract class COMAddin : IDTExtensibility2, Office.IRibbonExtensibility, Office.ICustomTaskPaneConsumer
    {
        ...
        /// </summary>
        protected PowerPoint.Application Application { get; private set; }
        ...
        {
            Tweaks.ApplyTweaks(Factory, this, Type, "PowerPoint");
        ...
        protected static bool SetTweakPersistenceEntry(Type addinType, string name, string value, bool throwException)
        {
            try
            {
                ...
                RegistryKey regKeyPPoint = null;
                if (registry.Value == RegistrySaveLocation.LocalMachine)
                    regKeyPPoint = Registry.LocalMachine.OpenSubKey(_addinOfficeRegistryKey + progID.Value, true);
                else
                    regKeyPPoint = Registry.CurrentUser.OpenSubKey(_addinOfficeRegistryKey + progID.Value, true);

                if (null == regKeyPPoint)
                    regKeyPPoint = Registry.CurrentUser.CreateSubKey(_addinOfficeRegistryKey + progID.Value);
                if (null == regKeyPPoint)
                    return false;
       ...
       /// <param name="methodKind">origin method where the error comes from</param>
        ...
        public static void RegisterFunction(Type type)
        {
            try                
            {
                ...
                RegistryKey regKeyPowerPoint = null;                
                if(location.Value == RegistrySaveLocation.LocalMachine)
                    regKeyPowerPoint = Registry.LocalMachine.OpenSubKey(_addinOfficeRegistryKey + progId.Value, true);
                else
                    regKeyPowerPoint = Registry.CurrentUser.OpenSubKey(_addinOfficeRegistryKey + progId.Value, true);

                regKeyPowerPoint.SetValue("LoadBehavior", addin.LoadBehavior);
                regKeyPowerPoint.SetValue("FriendlyName", addin.Name);
                regKeyPowerPoint.SetValue("Description", addin.Description);
                if(-1 != addin.CommandLineSafe)
                    regKeyPowerPoint.SetValue("CommandLineSafe", addin.CommandLineSafe);

                regKeyPowerPoint.Close();
