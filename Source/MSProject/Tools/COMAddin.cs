...
{
    /// <summary>
    /// NetOffice MS-Project COM Addin
    /// </summary>
    ...
    public abstract class COMAddin : COMAddinBase, IDTExtensibility2, Office.IRibbonExtensibility, Office.ICustomTaskPaneConsumer
    {
        ...
        /// </summary>
        protected internal MSProject.Application Application { get; private set; }
        ...
        void IDTExtensibility2.OnStartupComplete(ref Array custom)
        {
            try
            {
                Tweaks.ApplyTweaks(Factory, this, Type, "MSProject");
        ...
        protected static bool SetTweakPersistenceEntry(Type addinType, string name, string value, bool throwException)
        {
            try
            {
                ...
                RegistryKey regKeyProject = null;
                if (registry.Value == RegistrySaveLocation.LocalMachine)
                    regKeyProject = Registry.LocalMachine.OpenSubKey(_addinOfficeRegistryKey + progID.Value, true);
                else
                    regKeyProject = Registry.CurrentUser.OpenSubKey(_addinOfficeRegistryKey + progID.Value, true);

                if (null == regKeyProject)
                    regKeyProject = Registry.CurrentUser.CreateSubKey(_addinOfficeRegistryKey + progID.Value);
                if (null == regKeyProject)
                    return false;
       ...
       /// <param name="methodKind">origin method where the error comes from</param>
        ...
        public static void RegisterFunction(Type type)
        {
            try                
            {
                ...
                regKeyProject.SetValue("LoadBehavior", addin.LoadBehavior);
                regKeyProject.SetValue("FriendlyName", addin.Name);
                regKeyProject.SetValue("Description", addin.Description);
                if(-1 != addin.CommandLineSafe)
                    regKeyProject.SetValue("CommandLineSafe", addin.CommandLineSafe);
