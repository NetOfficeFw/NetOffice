...
namespace NetOffice.MSProjectApi.Tools
{
    ...
    public abstract class COMAddin : COMAddinBase, IOfficeCOMAddin
    {
        ...
        void NetOffice.Tools.Native.IDTExtensibility2.OnStartupComplete(ref Array custom)
        {
            try
            {
                Tweaks.ApplyTweaks(Factory, this, Type, "MS Project", IsLoadedFromSystem);
        ...
        public virtual void CTPFactoryAvailable(object CTPFactoryInst)
        {
            try
            {
                ...
                TaskPaneFactory = paneHandler.CreateCustomPanes<ITaskPane, MSProject.Application>(Factory, CTPFactoryInst, TaskPanes, TaskPaneInstances, OnError, Application);
            }
            catch (NetRuntimeSystem.Exception exception)
            {
                Factory.Console.WriteException(exception);
        ...
        protected static bool SetTweakPersistenceEntry(Type addinType, string name, string value, bool throwException)
        {
            return OfficeApi.Tools.COMAddin.SetTweakPersistenceEntry(
                                                          ApplicationIdentifiers.ApplicationType.MS_Project,
                                                          addinType,
                                                          name, value,
                                                          throwException);
        }
