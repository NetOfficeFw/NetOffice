    ...
    public abstract class COMAddin : IDTExtensibility2, Office.IRibbonExtensibility, Office.ICustomTaskPaneConsumer
    {
        ...
        public static void RegisterFunction(Type type)
        {
            try                
            {
                ...
                RegistryKey regKeyMSProject = null;
                
                if(location.Value == RegistrySaveLocation.LocalMachine)
                    regKeyMSProject = Registry.LocalMachine.OpenSubKey(_addinOfficeRegistryKey + progId.Value, true);
                else
                    regKeyMSProject = Registry.CurrentUser.OpenSubKey(_addinOfficeRegistryKey + progId.Value, true);

                regKeyMSProject.SetValue("LoadBehavior", addin.LoadBehavior);
                regKeyMSProject.SetValue("FriendlyName", addin.Name);
                regKeyMSProject.SetValue("Description", addin.Description);
                if(-1 != addin.CommandLineSafe)
                    regKeyMSProject.SetValue("CommandLineSafe", addin.CommandLineSafe);
